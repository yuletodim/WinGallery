...
using WinGallery.Web.Utils;
using WinGallery.Services.Utils;
using System;
...
namespace WinGallery.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                return View(model).WithError(AppConstants.LoginFailed);
            }
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    return RedirectToLocal(returnUrl).WithSuccess(AppConstants.LoginSuccess);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await this.emailServices.SendAsync(model.Email, emailBody, emailSubject);


                    return RedirectToAction("Login").WithSuccess(AppConstants.RegisterSuccess);
                }
                ...
                var translatedErrors = new IdentityResult(overrideErrors);

                AddErrors(translatedErrors);
            }
        ...
        {
            var language = this.Request.RequestContext.RouteData.Values["language"];
            AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
            return RedirectToAction("Index", "Home", new { langauge = language });
        }
